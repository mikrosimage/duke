...
Range::Range() : first(0), last(0) {}
Range::Range(size_t first, size_t last) : first(first), last(last) {}
bool Range::operator==(const Range& other) const { return first == other.first && last == other.last; }
Range Range::operator+(size_t frame) const { return Range(first + frame, last + frame); }
Range& Range::operator+=(size_t frame) {
  first += frame;
  last += frame;
  return *this;
}
size_t Range::count() const { return first <= last ? last - first + 1 : first - last + 1; }
...
bool consecutive(const Range& a, const Range& b) { return a.last + 1 == b.first; }
...
void Track::add(size_t frame, Clip&& clip) {
  if (clip.frames == 0) throw std::logic_error("Can't add a clip with zero frame");
...
bool Track::isClipAt(size_t frame) const { return clipContaining(frame) != end(); }

Track::const_iterator Track::nextClip(size_t frame) const { return upper_bound(frame); }

Track::const_iterator Track::previousClip(size_t frame) const {
  auto previous = findLess(*this, frame);
  if (previous == end()) return end();
  if (!contains(previous, frame)) return previous;
  if (previous == begin()) return end();
  return --previous;
}
...
Range Track::getRange() const {
  if (empty()) throw std::runtime_error("no range for empty track");
...
Ranges Track::getClipsRange() const {
  Ranges ranges;
  for (const TrackClip& pair : *this) ranges.push_back(rangeutils::getRange(pair));
  return ranges;
}

void Timeline::populateMediaAt(size_t frame, std::vector<MediaFrameReference>& frames) const {
  frames.clear();
  for (const Track& track : *this) frames.push_back(track.getMediaFrameReferenceAt(frame));
}
...
bool Timeline::empty() const {
  if (std::vector<Track>::empty()) return true;
  for (const Track& track : *this)
...
const Track* Timeline::findTrack(const char* pName) const {
  auto pFound = find_if(this->begin(), this->end(), [=](const Track & track)->bool { return track.name == pName; });
  if (pFound == end()) return nullptr;
  return &*pFound;
}
