...
CacheOverlay::CacheOverlay(const std::shared_ptr<GlyphRenderer>&pGlyphRenderer,
		const std::map<const IMediaStream*, std::vector<Range> > & state,
		const Timeline& timeline) :
				m_pGlyphRenderer(pGlyphRenderer), m_CacheState(state), m_timeline(timeline) {
}
...
void CacheOverlay::render(const Context& context) const {
	...
	// draw background
	drawRect(context.viewport.dimension,
			glm::ivec2(context.viewport.dimension.x, height), //size
			...
			if(clip.second.pStream) {
				...
				if(it != m_CacheState.end())
					for(const auto range : it->second) {
						...
						drawRect(context.viewport.dimension,
								glm::ivec2(rangeLength, height), //size
	...
	// draw cursor
	drawRect(context.viewport.dimension,
			glm::ivec2((frameLength<1.0)?1.0:frameLength, height), //size
	...
	drawText(*m_pGlyphRenderer, context.viewport, oss.str().c_str(), 5, height+10, 1.f, 1.f);
}
