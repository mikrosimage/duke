...
CmdCmd::CmdCmd(const Commands &cmds) : cmds(cmds), pCommandDescription(nullptr) {}

ManCmd::ManCmd(const Commands &cmds) : CmdCmd(cmds) {}

std::string ManCmd::execute() {
  if (pCommandDescription) return pCommandDescription->help;
...
std::string ManCmd::doParseArguments(std::istream &stream) {
  ...
  if (!pCommandDescription) return "unknown command " + token;
  return {};
}

ArgsCmd::ArgsCmd(const Commands &cmds) : CmdCmd(cmds) {}
...
std::string ArgsCmd::doParseArguments(std::istream &stream) {
  string token;
  if (!(stream >> token)) return "you must provide an argument";
  pCommandDescription = cmds.find(token);
  if (!pCommandDescription) return "unknown command " + token;
  return {};
}

SuggestCmd::SuggestCmd(const Commands &cmds) : CmdCmd(cmds) {}
...
std::string LsCmd::execute() { return "listing of '" + path + "'"; }

std::string LsCmd::doParseArguments(std::istream &stream) {
  if (!(stream >> path)) return string("'") + getString(CommandPlaceHolder::path) + "' expected";
  return {};
}
...
FunctionCmd::FunctionCmd(const std::function<void()> &func) : m_Function(func) {}
...
SuggestParam::SuggestParam(const Parameters &params) : params(params) {}
