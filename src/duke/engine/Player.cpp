...
Player::Player(const CmdLineParameters& parameters) : m_TextureCache(parameters) {}
...
void Player::setPlaybackTime(const Time time) { m_PlaybackTime = time; }
...
void Player::offsetPlaybackTime(const Time time) {
  ...
  if (m_TimelineRange == Range::EMPTY || offset == 0) return;
  ...
  switch (m_PlaybackMode) {
    case STOP:
      ...
      break;
    case LOOP:
      if (overshoot > (m_EndFrameTime - m_FirstFrameTime)) return;
...
void Player::setPlaybackSpeed(int speed) { m_PlaybackSpeed = speed; }

void Player::setPlaybackMode(const Mode mode) { m_PlaybackMode = mode; }

void Player::stop() { setPlaybackSpeed(0); }
...
FrameIndex Player::getCurrentFrame() const { return m_PlaybackTime / m_FrameDuration; }

Time Player::getPlaybackTime() const { return m_PlaybackTime; }

FrameDuration Player::getFrameDuration() const { return m_FrameDuration; }

int Player::getPlaybackSpeed() const { return m_PlaybackSpeed; }

Player::Mode Player::getPlaybackMode() const { return m_PlaybackMode; }

const Timeline& Player::getTimeline() const { return m_TextureCache.getTimeline(); }

LoadedTextureCache& Player::getTextureCache() { return m_TextureCache; }
