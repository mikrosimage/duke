...
template <typename C>
typename C::const_iterator findLessOrEquals(const C& container, const typename C::key_type& key) {
  const auto end = container.end();
  if (container.empty()) return end;
  auto bound = container.upper_bound(key);
  return bound == container.begin() ? end : --bound;
}

template <typename C>
typename C::const_iterator findLess(const C& container, const typename C::key_type& key) {
  const auto end = container.end();
  if (container.empty()) return end;
