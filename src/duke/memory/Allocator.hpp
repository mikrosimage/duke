...
struct BigAlignedBlock : public Allocator {
  ...
  virtual const char* name() const { return "BigAlignedBlock"; }
  virtual size_t alignment() const { return PAGE_SIZE; }
...
struct Malloc : public Allocator {
  virtual void* malloc(const size_t size) const { return std::malloc(size); }
  virtual void free(void* pBuffer) const { std::free(pBuffer); }
  virtual size_t alignment() const { return 1; }
  virtual const char* name() const { return "Malloc"; }
};
...
struct AlignedMalloc : public Allocator {
  ...
  virtual size_t alignment() const { return PAGE_SIZE; }
  virtual const char* name() const { return "AlignedMalloc"; }
};
...
struct New : public Allocator {
  virtual void* malloc(const size_t size) const { return new char[size]; }
  virtual void free(void* pBuffer) const { delete[] reinterpret_cast<char*>(pBuffer); }
  virtual size_t alignment() const { return 1; }
  virtual const char* name() const { return "New"; }
};
...
struct Virtual : public Allocator {
  ...
  virtual void free(void* pBuffer) const { ::VirtualFree(pBuffer, 0, MEM_RELEASE); }
  virtual size_t alignment() const { return PAGE_SIZE; }
  virtual const char* name() const { return "Virtual"; }
};
#endif
