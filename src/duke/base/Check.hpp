...
template <typename T>
inline T& _checkNotNull(const char* cond, const char* func, const char* file, const int line, T& ptr) {
  if (DUKE_UNLIKELY(!ptr)) LOG_FATAL(cond, func, file, line);
  return ptr;
}

#define CHECK_NOTNULL(X) _checkNotNull(#X " must not be nullptr", __func__, __FILE__, __LINE__, X)
