...
bool isOdd(int64_t count) { return (count & 0x01) > 0; }
...
double getCycleValue(const AnimationData& data, int64_t time) {
  ...
  if (!isInfinite && repeatedCount > data.repeatCount) return 1;
...
struct quadratic {
  static inline double apply(double t) { return t * t; }
};
struct cubic {
  static inline double apply(double t) { return quadratic::apply(t) * t; }
};
struct quartic {
...
struct quintic {
  static inline double apply(double t) { return quartic::apply(t) * t; }
};
struct sine {
  static inline double apply(double t) { return -::cos(t * kPi_2) + 1; }
};
struct expo {
  static inline double apply(double t) { return (t == 0 || t == 1.0) ? t : ::pow(2.0, 10 * (t - 1)) - 0.001; }
};
struct circ {
  static inline double apply(double t) { return -(::sqrt(1 - t * t) - 1); }
};
...
struct back {
  static inline double apply(double t, double overshoot) { return t * t * ((overshoot + 1) * t - overshoot); }
};

template <typename T, typename... Params>
struct in {
  static inline double apply(double t, Params... args) { return T::apply(t, args...); }
};
template <typename T, typename... Params>
struct out {
  static inline double apply(double t, Params... args) { return 1 - T::apply(1 - t, args...); }
};
template <typename T, typename... Params>
struct inout {
  static inline double apply(double t, Params... args) {
    if (t <= .5) return in<T, Params...>::apply(t * 2, args...) / 2;
...
struct outin {
  static inline double apply(double t, Params... args) {
    if (t <= .5) return out<T, Params...>::apply(t * 2, args...) / 2;
