...
DukeGLFWWindow *safeGetDukeWindow(GLFWwindow *pWindow) {
  ...
  if (pFound == glfwToDukeWindowMap.end()) throw std::runtime_error("Trying to reach an inexistent window");
  return pFound->second;
}
void onWindowCloseFun(GLFWwindow *pWindow) {
  auto pDukeWindow = safeGetDukeWindow(pWindow);
  if (pDukeWindow->windowCloseCallback) pDukeWindow->windowCloseCallback();
}
void onWindowFocusFun(GLFWwindow *pWindow, int focus) {
  auto pDukeWindow = safeGetDukeWindow(pWindow);
  if (pDukeWindow->windowFocusCallback) pDukeWindow->windowFocusCallback(focus);
}
void onWindowResizeFun(GLFWwindow *pWindow, int width, int height) {
  auto pDukeWindow = safeGetDukeWindow(pWindow);
  if (pDukeWindow->windowResizeCallback) pDukeWindow->windowResizeCallback(width, height);
}
void onWindowRefreshFun(GLFWwindow *pWindow) {
  auto pDukeWindow = safeGetDukeWindow(pWindow);
  if (pDukeWindow->windowRefreshFunCallback) pDukeWindow->windowRefreshFunCallback();
}
void onMouseButtonFun(GLFWwindow *pWindow, int buttonId, int buttonState, int modifier) {
  auto pDukeWindow = safeGetDukeWindow(pWindow);
  if (pDukeWindow->mouseButtonCallback) pDukeWindow->mouseButtonCallback(buttonId, buttonState);
}
void onMousePosFun(GLFWwindow *pWindow, double x, double y) {
  auto pDukeWindow = safeGetDukeWindow(pWindow);
  if (pDukeWindow->mousePosCallback) pDukeWindow->mousePosCallback(x, y);
}
void onMouseWheelFun(GLFWwindow *pWindow, double x, double y) {
  auto pDukeWindow = safeGetDukeWindow(pWindow);
  if (pDukeWindow->scrollCallback) pDukeWindow->scrollCallback(x, y);
}
void onKeyFun(GLFWwindow *pWindow, int keyId, int scancode, int keyState, int modifier) {
  auto pDukeWindow = safeGetDukeWindow(pWindow);
  if (pDukeWindow->keyCallback) pDukeWindow->keyCallback(keyId, keyState);
}
void onCharFun(GLFWwindow *pWindow, unsigned int unicodeCodePoint) {
  auto pDukeWindow = safeGetDukeWindow(pWindow);
  if (pDukeWindow->charCallback) pDukeWindow->charCallback(unicodeCodePoint);
}

}  // namespace

DukeGLFWApplication::DukeGLFWApplication() {
  if (!glfwInit()) throw std::runtime_error("Unable to initialize OpenGL");
...
DukeGLFWApplication::~DukeGLFWApplication() { glfwTerminate(); }

GLFWwindow *DukeGLFWApplication::createRawWindow(int width, int height, const char *title, GLFWmonitor *monitor,
                                                 GLFWwindow *share) {
  GLFWwindow *pWindow = glfwCreateWindow(width, height, title, monitor, share);
  if (!pWindow) throw std::runtime_error("Unable to open window");
  return pWindow;
}

DukeGLFWWindow::DukeGLFWWindow(GLFWwindow *pWindow) : m_pWindow(pWindow) {
  if (!m_pWindow) throw std::runtime_error("Illegal creation of nullptr Window");
...
GLFWwindow *DukeGLFWWindow::getHandle() { return m_pWindow; }
