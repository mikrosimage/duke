...
struct DescriptorSet {
  T &add(T &&value) { return m_Map.insert(std::make_pair(value.name, std::move(value))).first->second; }
  ...
  std::vector<std::string> suggest(const std::string &start) const {
    ...
    for (const auto &pair : m_Map)
      if (startsWith(pair.first, start)) suggestions.emplace_back(pair.first);
    return suggestions;
  }
