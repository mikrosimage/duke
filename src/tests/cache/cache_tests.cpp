...
BOOST_AUTO_TEST_CASE( fullButHigherPriorityDiscardsRequested )
{
    ...
    BOOST_ASSERT( cache.put(0,3,-1) ); // pushing 0 fills the cache
    BOOST_ASSERT( ! cache.isFull() ); // not full, 1 is not there

    BOOST_ASSERT( cache.put(1,1,-1) );// 1 has higher priority and must remove 0
    BOOST_ASSERT( ! cache.isFull() );// no full
    BOOST_ASSERT( ! cache.contains(0) );
