...
struct SmartCache::Impl : private boost::noncopyable {
    Impl(const size_t threads, const uint64_t limit, const ImageDecoderFactory& factory) :
        m_CacheActivated(limit > 0), m_ImageFactory(factory), m_LookAheadCache(limit), m_LastFrame(-1), m_LastSpeed(-1), m_pLastHelper(NULL) {

        if (m_CacheActivated && threads == 0) {
    ...
    inline bool get(uint64_t hash, ImageHolder & imageHolder) const {
        ...
        if (m_CacheActivated) {
            ...
            if (m_LookAheadCache.get(id, data, m_AvailableKeys))
                loaded = true;
            else
...
private:
    ...
    inline void displayQueueState(const uint64_t &currentHash) const {
        ...
        boost::mutex::scoped_lock lock(m_LookAheadCache.m_CacheMutex);
        cout << '[';
