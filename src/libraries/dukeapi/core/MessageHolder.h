...
private:
    boost::unordered_map<duke::protocol::MessageType_Type, const google::protobuf::Message*> m_TypeToDefaultInstance;
    boost::unordered_map<const google::protobuf::Descriptor*, duke::protocol::MessageType_Type> m_DescriptorToType;
    void addPair(const ::duke::protocol::MessageType_Type type, const google::protobuf::Message& defaultMsg) {
...
public:
    MessageTypeHelper() {
        ...
        addPair(MessageType_Type_INIT_RENDERER, duke::protocol::Renderer::default_instance());
    ...
    std::unique_ptr<google::protobuf::Message> newMessage(const duke::protocol::MessageType_Type type) const {
    ...
    duke::protocol::MessageType_Type typeOf(const google::protobuf::Message& msg) const {
...
private:
    ...
    const ::duke::protocol::MessageType_Action m_Operation;
public:
    MessageCopy(std::unique_ptr<google::protobuf::Message>& pMessage, ::duke::protocol::MessageType_Action action) :
        m_pMessage(std::move(pMessage)), m_Operation(action) {
        if (!m_pMessage)
            throw std::runtime_error("Cannot use a NULL message");
    }

    MessageCopy(const ::google::protobuf::Message& message, ::duke::protocol::MessageType_Action action) :
        m_pMessage(MessageHelper::instance.copy(message)), m_Operation(action) {
    }

    ::duke::protocol::MessageType_Type type() const {
        return MessageHelper::instance.typeOf(message());
    }

    inline ::duke::protocol::MessageType_Action operation() const {
        return m_Operation;
    }

    inline ::duke::protocol::MessageType messageType() const {
        ::duke::protocol::MessageType typeMessage;
