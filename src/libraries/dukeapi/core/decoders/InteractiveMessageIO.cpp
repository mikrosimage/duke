...
bool InteractiveMessageIO::tryPop(SharedMessage& holder) {
    return m_ToApplicationQueue.tryPop(holder);
}

void InteractiveMessageIO::waitPop(SharedMessage& holder) {
    m_ToApplicationQueue.waitPop(holder);
}

void InteractiveMessageIO::push(const SharedMessage& holder) {
    if (!holder)
    ...
    const Event& event = holder->message<Event> ();
    switch (event.type()) {
