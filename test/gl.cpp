...
struct DummyBindable : public duke::gl::IBindable {
  virtual void bind() const { ++boundCount; }
  virtual void unbind() const { ++unboundCount; }
  mutable size_t boundCount = 0;
  mutable size_t unboundCount = 0;
};

Binder<DummyBindable> scope_bind(const DummyBindable& bindable) { return Binder<DummyBindable>(&bindable); }
